...
class LinearModel:
    ...
    def activation(self, x):
        ...
        """
        # what about int???
        if isinstance(x, float) or isinstance(x, np.float64) or isinstance(x, np.float32) or isinstance(x, int):
            return 1 / (1 + np.exp(-1 * x))
        else:
            # raise the error
            raise ValueError("Activation function: Suppose pass a float value")
    ...
    def forward(self, inputs):
        ...
        """
        copied_arr = inputs.copy()
        copied_arr *= self.weights[:-1]
        return self.activation(copied_arr.sum() + self.weights[-1])
